#!/usr/bin/env python3
# Этот скрипт использует генератор для чтения файла по одной строке за раз и измеряет пиковое использование памяти и время выполнения
...
def main():
    if len(sys.argv) != 2:
        print("Hапиши скрипт и какой файл открыть")
        return
